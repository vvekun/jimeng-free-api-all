...
export async function generateVideo(
  ...
  await notifySubmitted(callbackContext, `视频生成任务已提交，history_id: ${historyId}`);
  ...
  while (status === 20 && retryCount < maxRetries) {
    try {
      ...
      // 发送进度回调通知
      await notifyProcessing(callbackContext, Math.min(90, (retryCount / maxRetries) * 100), `视频生成中，第 ${retryCount + 1} 次查询，状态: ${status}`);
      ...
      if (status === 30) {
        const errorMsg = failCode === 2038 ? "内容被过滤" : `生成失败，错误码: ${failCode}`;
        await notifyFailed(callbackContext, failCode === 2038 ? 'CONTENT_FILTERED' : 'GENERATION_FAILED', errorMsg);
  ...
  if (retryCount >= maxRetries && status === 20) {
    ...
    await notifyFailed(callbackContext, 'TIMEOUT', '获取视频生成结果超时');
  ...
  if (itemId) {
    try {
      ...
      if (hqVideoUrl) {
        ...
        await notifyCompleted(callbackContext, [hqVideoUrl], '视频生成成功（高质量）');
        return hqVideoUrl;
      }
    } catch (error) {
  ...
  if (!videoUrl) {
    ...
    if (item_list?.[0]?.video?.play_url) {
      ...
      await notifyFailed(callbackContext, 'NO_VIDEO_URL', '未能获取视频URL');
  ...
  await notifyCompleted(callbackContext, [videoUrl], '视频生成成功');
  return videoUrl;
}
...
export async function generateSeedanceVideo(
  ...
  await notifySubmitted(callbackContext, `Seedance 视频生成任务已提交，history_id: ${historyId}`);
  ...
  while (status === 20 && retryCount < maxRetries) {
    try {
      ...
      await notifyProcessing(callbackContext, Math.min(90, (retryCount / maxRetries) * 100), `Seedance 视频生成中，第 ${retryCount + 1} 次查询，状态: ${status}`);
  ...
  if (retryCount >= maxRetries && status === 20) {
    await notifyFailed(callbackContext, 'TIMEOUT', 'Seedance 视频生成超时');
  ...
  if (seedanceItemId) {
    try {
      ...
      if (hqVideoUrl) {
        ...
        await notifyCompleted(callbackContext, [hqVideoUrl], 'Seedance 视频生成成功（高质量）');
        return hqVideoUrl;
      }
    } catch (error) {
  ...
  if (!videoUrl) {
    await notifyFailed(callbackContext, 'NO_VIDEO_URL', 'Seedance 未能获取视频URL');
  ...
  await notifyCompleted(callbackContext, [videoUrl], 'Seedance 视频生成成功');
  return videoUrl;
}
