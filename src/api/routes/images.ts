...
export default {
  ...
  post: {
    "/generations": async (request: Request) => {
      ...
      // 创建回调上下文
      const callbackContext = createCallbackContext(callback_url, 'image', model || 'jimeng-4.5');
      ...
      if (callbackContext) {
        ...
        (async () => {
          try {
            ...
            logger.info(`异步图片生成完成: task_id=${callbackContext.taskId}, 生成 ${imageUrls.length} 张图片`);
          } catch (err) {
            logger.error(`异步图片生成失败: task_id=${callbackContext.taskId}, error=${err.message}`);
    ...
    "/compositions": async (request: Request) => {
      ...
      const {
        ...
        sample_strength: sampleStrength,
        response_format,
        callback_url: compositionCallbackUrl,
      } = request.body;
      ...
      // 创建回调上下文
      const compositionCallbackContext = createCallbackContext(compositionCallbackUrl, 'image', model || 'jimeng-4.5');

      // 如果有 callback_url，立即返回 task_id，后台异步执行生成
      if (compositionCallbackContext) {
        (async () => {
          try {
            const resultUrls = await generateImageComposition(model, prompt, images, {
              ...
              negativePrompt,
              intelligentRatio: finalIntelligentRatio,
            }, token, compositionCallbackContext);
            logger.info(`异步图生图完成: task_id=${compositionCallbackContext.taskId}, 生成 ${resultUrls.length} 张图片`);
          } catch (err) {
            logger.error(`异步图生图失败: task_id=${compositionCallbackContext.taskId}, error=${err.message}`);
            await notifyFailed(compositionCallbackContext, 'GENERATION_ERROR', err.message);
          }
        })();

        return {
          created: util.unixTimestamp(),
          task_id: compositionCallbackContext.taskId,
          status: "submitted",
          message: "任务已提交，生成结果将通过回调通知推送",
        };
      }
